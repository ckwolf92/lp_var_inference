...
k = [2 5 10];                           % Values of k to plot (for joint Wald confidence ellipsoid)
...
for i_k=1:n_k
    ...
    hold on;
    for i_M=1:n_M
        plot(se_ratio,wc_prob_wald(i_M,:,i_k),linestyles{i_M},'LineWidth',1);
    end
    hold off;
    the_xlim = xlim;
    line(the_xlim,[1 1]*(1-signif),'Color','k','LineStyle','-'); % Mark nominal coverage level on vertical axis
    ...
    set(gca,'FontSize',12);
    title(sprintf('%s%d%s','$k=',k(i_k),'$'),'Interpreter','Latex','FontSize',14);
    if i_k==n_k
        label_legend(M);
    end
end
...
function label_legend(M)
    legend(strcat('M=', arrayfun(@num2str, M, 'UniformOutput', 0)),'Location','SouthEast','FontSize',12);
end