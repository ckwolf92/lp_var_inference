%% LP vs VAR INFERENCE: QUANT SIMULATIONS
...
settings.est.type = 'quant'; % estimation methods type
...
settings.est.har       = @(Y,bw) ewc(Y,bw); % HAR estimator
settings.est.har_bw    = @(T) round(0.4*T.^(2/3)); % HAR bandwidth
settings.est.har_cv    = @(bw) tinv(1-settings.alpha/2,bw); % HAR critical value
...
spec_shared = {'alpha',settings.est.alpha, ...
               ...
               'har_bw',settings.est.har_bw, ...
               'har_cv',settings.est.har_cv};
...
cis_lower = zeros(numdgp, numspec, numhorz, 4, numrep); % 4th index = type of CI: 
                                     % i) delta method, 
                                     % ii) Efron, 
                                     % iii) Hall, 
